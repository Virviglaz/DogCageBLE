...
#define DEVICE_NAME                     "CAGE"                               /**< Name of device. Will be included in the advertising data. */
...
#define START_STRING                    ""                                /**< The string that will be sent over the UART when the application starts. */
...
char ReadSN_Passed = 0;
char DeviceName[31] = DEVICE_NAME;
char DeviceSN[8] = {0};
char CageStatus = 'N';

static void ChangeBLE_AdvName (char * Name);
...
static void advertising_init(void)
{
    ...
    manuf_data.data.p_data              = (uint8_t*)DeviceName;     
    manuf_data.data.size                = strlen(DeviceName);

    ble_uuid_t adv_uuids[] = {{0x1101, m_nus.uuid_type}};
...
static void UartPrint (char * data)
{
	while(*data)
		app_uart_put(* data++);
	app_uart_put('\n');
...
static void ChangeBLE_AdvName (char * Name)
{
	...
	sprintf(DeviceName, "%s %s %c", DEVICE_NAME, DeviceSN, CageStatus);
...
void uart_event_handle(app_uart_evt_t * p_event)
{
    ...
    {
        case APP_UART_DATA_READY:					/* DATA FROM MCU */
            ...
            {						
                ...
                if (err_code != NRF_ERROR_INVALID_STATE)
                    APP_ERROR_CHECK(err_code);
								...
								uint8_t bleAdvHandler(char * data);
								if (bleAdvHandler((char*)data_array)) { index = 0; break; }
...
uint8_t bleAdvHandler(char * data)
{
	if (strncmp(data, "BLEADV", 6)) return 0;
	
	char * ptr = strtok(data, " ");//'BLEADV'
	if (ptr == NULL) return 0;
								
	uint8_t Adv_changed = 0;
	
	ptr = strtok(NULL, " "); //SN
	if (ptr == NULL) return 1;

	// update device SN if needed
	if (strcmp(DeviceSN, ptr))
	{
		Adv_changed = 1;
		strcpy(DeviceSN, ptr);
	}
